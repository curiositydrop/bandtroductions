...
(function initNoAppAppHelp(){
  ...
  overlay.style.cssText='position:fixed;inset:0;z-index:10000;background:rgba(0,0,0,.78);display:grid;place-items:center;padding:16px';
  overlay.hidden=true;
  document.body.appendChild(overlay);

  const open=event=>{event.preventDefault();overlay.hidden=false;overlay.style.display='grid';document.getElementById('no-app-app-help-close')?.focus();};
  const close=()=>{overlay.hidden=true;overlay.style.display='none';link.focus();};
