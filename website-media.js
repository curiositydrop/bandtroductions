...
export function profileVideos(profile){
 ...
 return normalizeMedia({videos:candidates.filter(Boolean).map(v=>typeof v==='string'?{url:v}:v)}).videos;
}
export function applyMedia(input,previewUrls={}){
 ...
 const order=settings.layout==='music-first'?['home','music','about','photos','merch','contact']:settings.layout==='photo-first'?['home','photos','about','music','merch','contact']:['home','about','music','photos','merch','contact'];
 order.forEach(id=>{if(el(id))main.append(el(id));});
 const grid=el('photo-grid');if(!grid)return;grid.replaceChildren();
 for(const p of settings.photos){const src=previewUrls[p.id]||p.url;if(!p.visible||!src)continue;const figure=document.createElement('figure'),img=document.createElement('img');img.src=src;img.alt=p.caption||'Band photo';img.loading='lazy';figure.append(img);if(p.caption){const caption=document.createElement('figcaption');caption.textContent=p.caption;figure.append(caption);}grid.append(figure);}
...
export function createMediaEditor({container,changed,preparePhoto,removePending,getPreviewUrl}){
 ...
 return {
  fill(settings,profile){const s=normalizeMedia(settings);photos=structuredClone(s.photos);videos=structuredClone(s.videos??profileVideos(profile));$('site-layout').value=s.layout;$('site-button-style').value=s.buttonStyle;drawPhotos();drawVideos();},
