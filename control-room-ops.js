...
const parsedLocation=p=>{
  if(!p)return {city:'',region:'',country:''};
  if(p.city||p.region||p.country)return {city:String(p.city||'').trim(),region:String(p.region||'').trim(),country:String(p.country||'').trim()};
  const raw=String(p.location||'').trim();if(!raw)return {city:'',region:'',country:''};
  ...
  if(['united states','usa','u.s.a.','us','u.s.','canada','united kingdom','uk','england','scotland','wales','ireland','australia'].includes(lower))return {city:'',region:'',country:raw};
  return {city:'',region:raw,country:''};
};
const needsLocationUpdate=p=>{const loc=parsedLocation(p);return !(loc.city&&loc.region&&loc.country)};
...
function install(){
  ...
  wrap.innerHTML=`
  <section class="cr-section" id="cr-needs-attention"><div class="cr-head"><div><p class="cr-kicker">Priority Queue</p><h2>Needs Attention</h2><div class="cr-muted">Anything that may need an admin decision lands here.</div></div></div><div class="cr-attention"><div class="cr-alert"><strong id="cr-incomplete">0</strong><span>Profile setup incomplete</span></div><div class="cr-alert"><strong id="cr-location-needed">0</strong><span>Location updates needed</span></div><div class="cr-alert"><strong id="cr-pending-prof">0</strong><span>Profiles awaiting approval</span></div><div class="cr-alert"><strong id="cr-pending-claim">0</strong><span>Legacy claims awaiting review</span></div></div></section>
...
function renderAttentionAndPulse(){
  ...
  const locationNeeded=profiles.filter(p=>needsLocationUpdate(p)).length;
  qs('#cr-incomplete').textContent=incomplete;qs('#cr-location-needed').textContent=locationNeeded;qs('#cr-pending-prof').textContent=pendingProf;qs('#cr-pending-claim').textContent=pendingClaim;
...
async function sendBroadcast(event){
  ...
  const recipients=users.filter(u=>{const p=profileForUid(u.id);if(audience==='all')return true;if(audience==='missing-location')return Boolean(p)&&needsLocationUpdate(p);return (p?.accountType||u.accountType||'fan')===audience});if(!recipients.length){status.textContent='No matching members found.';return}if(!confirm(`Send this notification to ${recipients.length} member${recipients.length===1?'':'s'}?`))return;
  const button=event.submitter;button.disabled=true;status.textContent=`Sending to ${recipients.length} members…`;
  try{for(let i=0;i<recipients.length;i+=400){const batch=writeBatch(db);recipients.slice(i,i+400).forEach(u=>{const ref=doc(collection(db,'notifications'));batch.set(ref,{recipientId:u.id,actorId:currentUser.uid,actorName:'BANDtroductions Admin',message,linkUrl:link,read:false,type:'admin-broadcast',emailRequested:true,createdAt:serverTimestamp()})});await batch.commit()}status.textContent=`Sent to ${recipients.length} members.`;qs('#cr-broadcast-message').value=''}catch(error){console.error(error);status.textContent=error?.code==='permission-denied'?'Firestore blocked the broadcast. Admin notification permissions need updating.':'Broadcast could not be sent.'}finally{button.disabled=false}
}
