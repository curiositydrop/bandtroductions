...
export function normalizeMedia(input={}){
 ...
 if(Array.isArray(input.videos)){out.videos=[];const seen=new Set();for(const [i,v] of input.videos.slice(0,12).entries()){if(!v||typeof v!=='object')continue;const yt=youtubeId(v.url),direct=imageUrl(v.url);const key=yt?'youtube:'+yt:direct?'upload:'+direct:'';if(!key||seen.has(key))continue;seen.add(key);out.videos.push({id:/^video_[a-zA-Z0-9_-]+$/.test(v.id)?v.id:'video_saved_'+i,url:yt?'https://www.youtube.com/watch?v='+yt:direct,title:String(v.title||'').slice(0,100),visible:v.visible!==false,kind:yt?'youtube':'upload'});}}
 return out;
}
...
export function createMediaEditor({container,changed,preparePhoto,prepareVideo,removePending,getPreviewUrl}){
 ...
 function drawVideos(){const list=$('library-video-list');list.replaceChildren();videos.forEach((v,i)=>{const row=document.createElement('div');row.className='library-row';const title=document.createElement('strong');title.textContent='Video '+(i+1)+(i===0?' · Featured when visible':'');row.append(title);const preview=getPreviewUrl(v.id)||v.url;if(v.kind==='youtube'){const note=document.createElement('p');note.textContent='Existing YouTube video';row.append(note);}else if(preview){const video=document.createElement('video');video.src=preview;video.controls=true;video.preload='metadata';video.style.maxWidth='220px';video.style.width='100%';row.append(video);}row.append(input('Title / caption',v.title,100,x=>{v.title=x;}),visibleControl(v));const controls=orderControls(row,videos,i,drawVideos);if(i>0)controls.append(button('Make featured',()=>{videos.splice(i,1);videos.unshift(v);v.visible=true;drawVideos();changed();}));controls.append(button('Remove',()=>{removePending(v.id);videos.splice(i,1);drawVideos();changed();}));list.append(row);});$('library-videos').disabled=videos.length>=12;}
 $('site-layout').onchange=changed;$('site-button-style').onchange=changed;
 $('library-videos').onchange=async()=>{const files=[...$('library-videos').files];if(!files.length)return;const status=document.getElementById('editor-status'),fields=document.getElementById('website-fields');if(videos.length+files.length>12){status.textContent='Choose fewer videos. This library holds up to 12.';$('library-videos').value='';return;}fields.disabled=true;section.dataset.processing='true';let count=0;try{for(const file of files){if(!['video/mp4','video/webm','video/quicktime'].includes(file.type)||file.size<=0||file.size>150*1024*1024)throw new Error('Choose MP4, WebM or MOV videos under 150 MB.');const id='video_'+crypto.randomUUID();await prepareVideo(file,id);videos.push({id,url:'',title:file.name.replace(/\.[^.]+$/,''),visible:true,kind:'upload'});count++;changed();}status.textContent='Videos ready. Preview before publishing.';}catch(error){status.textContent=error.message+(count?' '+count+' video(s) were added to your draft.':'');}finally{delete section.dataset.processing;fields.disabled=false;$('library-videos').value='';drawVideos();}};
 ...
 return {
  ...
  read(){return {layout:$('site-layout').value,buttonStyle:$('site-button-style').value,photos:structuredClone(photos),videos:structuredClone(videos)};},
...
export function websiteVideos(profile,settings={}){
 ...
 for(const v of normalizeMedia(settings).videos||[])merged.set(v.kind==='youtube'?'youtube:'+youtubeId(v.url):'upload:'+v.url,v);
