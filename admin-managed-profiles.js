...
function makeCard(profile){
  ...
  if(!profile.accountOnly){const view=document.createElement('a');view.className='auth-button auth-button-secondary';view.href=`profile.html?id=${encodeURIComponent(profile.id)}`;view.textContent='View Profile';const edit=document.createElement('a');edit.className='auth-button auth-button-secondary';edit.href=`profile-setup.html?adminProfile=${encodeURIComponent(profile.id)}`;edit.textContent='Edit Profile';const publish=document.createElement('button');publish.type='button';publish.className='auth-button';publish.textContent=profile.published===false?'Publish':'Unpublish';publish.addEventListener('click',async()=>{const next=profile.published===false;if(!confirm(`${next?'Publish':'Unpublish'} ${profile.displayName||'this profile'}?`))return;publish.disabled=true;try{await updateDoc(doc(db,'profiles',profile.id),{published:next,approvalStatus:next?'approved':profile.approvalStatus||'approved',updatedAt:serverTimestamp()})}catch(error){console.error(error);alert('The profile status could not be changed.');publish.disabled=false}});actions.append(view,edit,publish)}
