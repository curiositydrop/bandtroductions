...
const parsedLocation=p=>{if(!p)return {city:'',region:'',country:''};if(p.city||p.region||p.country)return {city:String(p.city||'').trim(),region:String(p.region||'').trim(),country:String(p.country||'').trim()};const raw=String(p.location||'').trim();if(!raw)return {city:'',region:'',country:''};const parts=raw.split(',').map(x=>x.trim()).filter(Boolean);if(parts.length>=3)return {city:parts[0],region:parts.slice(1,-1).join(', '),country:parts[parts.length-1]};if(parts.length===2)return {city:parts[0],region:parts[1],country:''};const lower=raw.toLowerCase();if(['united states','usa','u.s.a.','us','u.s.','canada','united kingdom','uk','england','scotland','wales','ireland','australia'].includes(lower))return {city:'',region:'',country:raw};return {city:'',region:raw,country:''}};
...
function makeCard(profile){
  ...
  const badge=document.createElement('span');const locationNeeded=needsLocationUpdate(profile);badge.className=`managed-badge ${locationNeeded?'incomplete':state}`;badge.textContent=locationNeeded?'location update':state==='incomplete'?'setup incomplete':state;
  ...
  if(!profile.accountOnly){
    ...
    const remove=document.createElement('button');remove.type='button';remove.className='auth-button managed-delete';remove.textContent='Delete Profile Listing';
    remove.addEventListener('click',async()=>{
      ...
      try{await deleteDoc(doc(db,'profiles',profile.id));alert(`The profile listing for "${label}" was deleted. The member login and other content were left intact.`)}
...
function render(){
  ...
  const visible=records.filter(p=>{const hay=[p.displayName,p.email,p.location,p.genre,p.instruments,p.venueType,p.accountType,p.claimedByEmail].join(' ').toLowerCase();return(!term||hay.includes(term))&&(type==='all'||p.accountType===type)&&(state==='all'||(state==='location-needed'?needsLocationUpdate(p):statusFor(p)===state));}).sort((a,b)=>{const onlineDiff=Number(isOnline(b))-Number(isOnline(a));if(onlineDiff)return onlineDiff;return seconds(b.lastActiveAt||b.updatedAt||b.submittedAt||b.createdAt)-seconds(a.lastActiveAt||a.updatedAt||a.submittedAt||a.createdAt)});
