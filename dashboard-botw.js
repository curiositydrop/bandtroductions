...
if(panel){
  ...
  const imageFor=b=>b.image||b.profileImage||b.photo||b.logo||b.avatar||b.banner||'IMG_9383.jpeg';
  ...
  onValue(ref(db,'Bands'),snap=>{
    ...
    const winner=ranked[0],name=nameFor(winner.id,winner.b),img=imageFor(winner.b),url=urlFor(winner.id,winner.b);
    body.innerHTML=`<a href="${url}" style="display:block;text-decoration:none;color:inherit"><img src="${img}" alt="${name}" style="width:100%;aspect-ratio:16/10;object-fit:cover;border:1px solid #333;margin-bottom:8px" onerror="this.src='IMG_9383.jpeg'"><b style="display:block;color:var(--teal);font-size:1.05em;line-height:1.15">${name}</b><p style="color:var(--muted);margin:5px 0">${safeNumber(winner.b.votes)} votes · ${safeNumber(winner.b.likes)} likes</p></a><a class="btn" href="band-of-the-week.html">VIEW LEADERBOARD</a>`;
