...
export function normalizeMembers(input){
 ...
 return (Array.isArray(input)?input:[]).slice(0,16).filter(m=>m&&typeof m==='object').map((m,i)=>{
  ...
  return {id,name:String(m.name||'').trim().slice(0,100),instrument:String(m.instrument||'').trim().slice(0,100),photoUrl:photoUrl(m.photoUrl),showPhoto:m.showPhoto!==false};
 });
}
export function renderMembers(settings={},previewUrls={}){
 ...
 const heading=document.getElementById('member-heading');if(heading)heading.hidden=settings.sections?.meetBand===false;
 ...
 for(const m of normalizeMembers(settings.bandMembers)){
  ...
  const initials=document.createElement('span');initials.textContent='Photo';initials.setAttribute('aria-hidden','true');portrait.append(initials);
  ...
  const name=document.createElement('h3');name.textContent=m.name||'Member name';portrait.hidden=m.showPhoto===false;card.append(portrait,name);
  const role=document.createElement('p');role.textContent=m.instrument||'Instrument / role';card.append(role);
  grid.append(card);
 }
...
export function createMemberEditor({container,changed,preparePhoto,removePending,getPreviewUrl}){
 ...
 function draw(){
  ...
  for(const [i,m] of members.entries()){
   ...
   const visible=document.createElement('label'),box=document.createElement('input');box.type='checkbox';box.checked=m.showPhoto!==false;box.onchange=()=>{m.showPhoto=box.checked;changed();};visible.append(box,document.createTextNode('Show member photo'));row.append(visible);
...
export function initialMembers(profile,settings={}){
 ...
 if(matches.length&&!remainder)return normalizeMembers(matches.map((m,i)=>({id:'member_legacy_'+i,name:m[1].trim(),instrument:m[2]})));
 return Array.from({length:4},(_,i)=>({id:'member_empty_'+i,name:'',instrument:'',photoUrl:'',showPhoto:true}));
}
