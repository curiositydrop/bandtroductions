...
export async function initWebsiteEditor({profileId,profile,render}){
 ...
 function mount(user){
  ...
  const collapseEditorCards=()=>panel.querySelectorAll('.editor-grid > section').forEach(card=>{if(card.dataset.collapsibleReady)return;card.dataset.collapsibleReady='true';const heading=card.querySelector('h3');if(!heading)return;heading.style.cursor='pointer';heading.style.display='flex';heading.style.justifyContent='space-between';heading.style.alignItems='center';heading.setAttribute('role','button');heading.setAttribute('aria-expanded','false');const indicator=document.createElement('span');indicator.textContent='＋';indicator.setAttribute('aria-hidden','true');indicator.style.fontSize='28px';indicator.style.lineHeight='1';heading.append(indicator);[...card.children].filter(node=>node!==heading).forEach(node=>node.hidden=true);heading.onclick=()=>{const open=heading.getAttribute('aria-expanded')==='true';heading.setAttribute('aria-expanded',String(!open));indicator.textContent=open?'＋':'−';[...card.children].filter(node=>node!==heading).forEach(node=>node.hidden=open);};});collapseEditorCards();
