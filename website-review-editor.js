...
export function normalizeSettings(input={}){
 ...
 const settings={theme,tagline,heroBrightness,heroPosition,sections:{about:input.sections?.about!==false,music:input.sections?.music!==false,merch:input.sections?.merch!==false,meetBand:input.sections?.meetBand!==false},booking:{...bookingInput,enabled:bookingInput.enabled===true,rateCents:Number.isFinite(Number(bookingInput.rateCents))?Math.max(0,Math.round(Number(bookingInput.rateCents))):0,rateBasis:bookingInput.rateBasis==='member'?'member':'show',paymentPolicy,depositPercent:paymentPolicy==='deposit'?50:0},buttons:[]};
...
export function applyWebsiteStyle(input,profile={}){
 ...
 for(const [key,value] of Object.entries(s.theme))root.style.setProperty(`--site-${key}`,value);root.style.setProperty('--site-hero-brightness',String(s.heroBrightness));root.style.setProperty('--site-hero-position-x',s.heroPosition.x+'%');root.style.setProperty('--site-hero-position-y',s.heroPosition.y+'%');
...
export async function initWebsiteEditor({profileId,profile,render}){
 ...
 function mount(user){
  ...
  function settingsFromForm(){
   const s=structuredClone(draft);s.tagline=$('site-tagline').value.trim().slice(0,120);s.heroBrightness=Number($('site-hero-brightness').value);s.heroPosition={x:Number($('site-hero-x').value),y:Number($('site-hero-y').value)};s.theme={};for(const key of Object.keys(DEFAULTS))s.theme[key]=$(`site-${key}`).value;
