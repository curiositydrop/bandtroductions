...
export function normalizeSettings(input={}){
 ...
 const settings={theme,tagline,heroBrightness,sections:{about:input.sections?.about!==false,music:input.sections?.music!==false,merch:input.sections?.merch!==false,meetBand:input.sections?.meetBand!==false},booking:{...bookingInput,enabled:bookingInput.enabled===true,rateCents:Number.isFinite(Number(bookingInput.rateCents))?Math.max(0,Math.round(Number(bookingInput.rateCents))):0,rateBasis:bookingInput.rateBasis==='member'?'member':'show',paymentPolicy,depositPercent:paymentPolicy==='deposit'?50:0},buttons:[]};
...
export function applyWebsiteStyle(input,profile={}){
 ...
 for(const [key,value] of Object.entries(s.theme))root.style.setProperty(`--site-${key}`,value);root.style.setProperty('--site-hero-brightness',String(s.heroBrightness));
...
export async function initWebsiteEditor({profileId,profile,render}){
 ...
 function mount(user){
  ...
  <section><h3>Hero</h3><label>Tagline<input type="text" id="site-tagline" maxlength="120" placeholder="Heavy music. No apologies."></label><p>This appears under the band name. Our wording is only a suggestion—make it yours or leave it blank.</p></section>
  <section><h3>Hero</h3><label>Background brightness <input type="range" id="site-hero-brightness" min="0.70" max="1.50" step="0.01"></label><output id="site-hero-brightness-value"></output><p>Adjust the hero image until it looks right on your page.</p></section>
  ...
  function settingsFromForm(){
   const s=structuredClone(draft);s.tagline=$('site-tagline').value.trim().slice(0,120);s.heroBrightness=Number($('site-hero-brightness').value);s.theme={};for(const key of Object.keys(DEFAULTS))s.theme[key]=$(`site-${key}`).value;
  ...
  form.addEventListener('input',event=>{if(event.target.id==='site-hero-brightness')$('site-hero-brightness-value').textContent=Math.round(Number(event.target.value)*100)+'%';setDirty();});
