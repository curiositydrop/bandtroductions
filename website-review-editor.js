...
export async function initWebsiteEditor({profileId,profile,render}){
 ...
 function mount(user){
  ...
  const collapseEditorCards=()=>panel.querySelectorAll('.editor-grid > section').forEach(card=>{if(card.dataset.collapsibleReady)return;card.dataset.collapsibleReady='true';const heading=card.querySelector('h3');if(!heading)return;heading.style.cursor='pointer';heading.setAttribute('role','button');heading.setAttribute('aria-expanded','false');[...card.children].filter(node=>node!==heading).forEach(node=>node.hidden=true);heading.onclick=()=>{const open=heading.getAttribute('aria-expanded')==='true';heading.setAttribute('aria-expanded',String(!open));[...card.children].filter(node=>node!==heading).forEach(node=>node.hidden=open);};});collapseEditorCards();
