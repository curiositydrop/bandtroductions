...
export function normalizeSettings(input={}){
 ...
 const backgroundImageUrl=safeButtonUrl(input.backgroundImageUrl);const settings={theme,tagline,heroBrightness,heroPosition,backgroundImageUrl,sections:{about:input.sections?.about!==false,music:input.sections?.music!==false,merch:input.sections?.merch!==false,meetBand:input.sections?.meetBand!==false},booking:{...bookingInput,enabled:bookingInput.enabled===true,rateCents:Number.isFinite(Number(bookingInput.rateCents))?Math.max(0,Math.round(Number(bookingInput.rateCents))):0,rateBasis:bookingInput.rateBasis==='member'?'member':'show',paymentPolicy,depositPercent:paymentPolicy==='deposit'?50:0},buttons:[]};
