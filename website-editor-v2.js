...
export function normalizeSettings(input={}){
 ...
 const settings={theme,sections:{about:input.sections?.about!==false,music:input.sections?.music!==false,merch:input.sections?.merch!==false,meetBand:input.sections?.meetBand!==false},buttons:[]};
...
export async function initWebsiteEditor({profileId,profile,render}){
 ...
 function mount(user){
  ...
  function fill(){memberEditor.fill(saved);mediaEditor.fill(saved,profile);for(const key of Object.keys(DEFAULTS))$(`site-${key}`).value=saved.theme[key];for(const id of ['about','music','merch'])$(`show-${id}`).checked=saved.sections[id];$('show-meet-band').checked=saved.sections.meetBand!==false;rows.replaceChildren();saved.buttons.forEach(addRow);$('add-button').disabled=rows.children.length>=6;draft=structuredClone(saved);$('site-banner').value='';$('site-image').value='';}
  fill();
  ...
  function settingsFromForm(){
   ...
   s.sections={};for(const id of ['about','music','merch'])s.sections[id]=$(`show-${id}`).checked;s.sections.meetBand=$('show-meet-band').checked;
