import { normalizeMedia, profileVideos, applyMedia, createMediaEditor } from './website-media.js?v=1';
...
export function websiteProfile(profile,settings){const s=normalizeSettings(settings);return {...profile,...(s.videos?{mediaLink:'',additionalMedia:s.videos.filter(v=>v.visible!==false),mediaItems:[]}:{}),...(s.bannerImageUrl?{bannerImageUrl:s.bannerImageUrl}:{}),...(s.imageUrl?{imageUrl:s.imageUrl}: {})};}
export function applyWebsiteStyle(input){
 const s=normalizeSettings(input),root=document.documentElement,el=id=>document.getElementById(id);applyMedia(s);
...
export async function initWebsiteEditor({profileId,profile,render}){
 ...
 function mount(user){
  ...
  function paint(s,withDraftImages=false){render(profile,s);if(withDraftImages)applyMedia(s,urls);if(withDraftImages){if(urls.bannerImageUrl)document.getElementById('cover').src=urls.bannerImageUrl;if(urls.imageUrl){document.getElementById('portrait').src=urls.imageUrl;document.getElementById('portrait').hidden=false;}}}
