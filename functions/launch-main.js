...
// Keep the Website + Merch plan exports, then add a narrowly scoped admin-only
...
async function grantPartner(profileSnapshot) {
  ...
  if (!['band', 'musician'].includes(type)) {
    throw new HttpsError('failed-precondition', `${profile.displayName || profileId} is not a band or musician profile.`);
  }
